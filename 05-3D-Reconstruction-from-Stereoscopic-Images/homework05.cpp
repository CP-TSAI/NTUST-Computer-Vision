...
void normalized_4x1(Eigen::MatrixXd& mat)
{
...
void get_camera_parameters(
	Eigen::MatrixXd& l_cam_k,
	Eigen::MatrixXd& r_cam_k,
	Eigen::MatrixXd& l_cam_rt,
	Eigen::MatrixXd& r_cam_rt,
...
int main()
{
	...
	get_camera_parameters(L_Camera_K, R_Camera_K, L_Camera_RT, R_Camera_RT, Fundamental_Matrix);
	...
	for (int imgName = 0; imgName <= 363; imgName++)
	{
		...
		// preprocess (make it into binary)
		...
		// if (imgName == 120)
		// {
		// 	cv::imwrite("gray_L.jpg", gray_L);
		// 	cv::imwrite("gray_R.jpg", gray_R);
		// 	std::cout << "saved gray image" << std::endl;
		// 	return 0;
		// }
	...
	for (unsigned i = 0; i < L_point.size(); i++)
	{
		...
		Eigen::MatrixXd L_point_test(3, 1);
		Eigen::MatrixXd R_point_test(3, 1);
